...
### 2.5 O que o deploy executa (Nixpacks / Docker)
...
Se `/health/db` falhar, revise `DATABASE_URL` e SSL.

### 2.7 Popular o banco (seed) — uma vez

O login precisa do **usuário** no Postgres (criado pelo seed):

- **Opção A — máquina local:** no `backend/.env`, `DATABASE_URL` igual à produção (cuidado para não commitar).  
...
|---------|----------------|
| **`npm error signal SIGTERM`** / processo morto ao subir | Timeout de health check, OOM ou redeploy. Confira **Logs**; migrações rodam no **start** (`start-prod.js`). Se migrate demorar muito, aumente tolerância do health check no Railway. |
| *Tenant or user not found* | Usar URI **Direct** do Supabase em `DATABASE_URL`. |
...
| Build falha em `migrate deploy` (Docker) | O projeto não deve rodar migrate no build; use o `Dockerfile` / `nixpacks.toml` do repositório (só `generate` no build). Migrações no **start**. |
